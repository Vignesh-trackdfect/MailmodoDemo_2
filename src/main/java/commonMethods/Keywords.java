...
public class Keywords extends ATUReports implements MailmodoLocators {
...
//			webElement.sendKeys(keysToSend, Keys.ENTER);
	...
	public String getTextJavascript(WebDriver driver, String xpath) {
		...
		//String text="" ;
        String elementText = (String) ((JavascriptExecutor) driver).executeScript("return arguments[0].innerText;", webElement);

		add(driver, "The value ' " + elementText + " ' is retrieved for the element ' " + values[0] + "'", LogAs.PASSED, true,
				values[0]);
		return elementText;
	...
	public void scrollBottom(WebDriver driver) {
		try {
			...
			JavascriptExecutor jse = (JavascriptExecutor) driver;
			jse.executeScript("window.scroll(0,25)", "");
			waitTime(driver, "5");
...
public List<String> getContactsList1(WebDriver driver,String name){
	...
	public void elementnotvisible(WebDriver driver, String xpath) {
		...
		try {
			...
			for (int i = 1; i <= 100; i++) {
				...
				//System.out.println("Import popup not visible");
			}
	...
	public void refreshPage1(WebDriver driver) {
		try {
	 ...
	 public void Upload_PDF_file(WebDriver driver,String Location) {
	  ...
	  public void verifyElementText(WebDriver driver, String xpath,String expectedtext) {
		...
		public void searchExistList(WebDriver driver,String existList) {
			...
			driver.findElement(By.xpath("//div[contains(@label,'"+existList+"')]")).click();
			wait(driver,"1");
		}
		//div[contains(@label,'"+existList+"')]
		
		public  void clickPreview(WebDriver driver,String Value) {
			
			String xpath="(//*[contains(text(),'"+Value+"')]//following::button)[1]";
		}
